...
def posage_opt(Ls1, Ls2, Delta_x=1.0):
    """
    Trouve les abscisses des points permettant un posage optimal entre deux pièces discrétisées.
    ...
    Returns:
        list: Une liste contenant les indices des points associés au posage optimal.
    """
    n = len(Ls1)
    posage_min = float('inf')  # Distance minimale initiale
    posage_indices = None      # Indices associés au posage optimal

    # Double boucle pour examiner tous les couples de points
    for i in range(n):
        for j in range(i + 1, n):
            # Calculer les distances algébriques min et max pour ce couple
            max_dist, min_dist = posage(Ls1, Ls2, i, j, Delta_x)
            
            # Critère de posage optimal : chercher la distance minimale globale
            if max_dist < posage_min:
                posage_min = max_dist
                posage_indices = [i, j]

    return posage_indices

# Exemple d'utilisation
P1 = [2., 3., 2., 3.]  # Pièce 1
P2 = [-3., -1., -2., -1.]  # Pièce 2 retournée

resultat = posage_opt(P1, P2)
print("Indices du posage optimal:", resultat)